...
public class StringEscapeTest {
	...
	public void unescapePartialQuotedTest() {
		...
		for(int i = 0, size = inputs.size(); i < size; i++) {
			int index = StringEscape.unescapePartialQuoted(inputs.get(i), offset, inputs.get(i).length() - offset, '\\', '"', ',', ']', false, dst);
			Assert.assertEquals(expect.get(i), dst.toString());
	...
	public void escapeXml() {
		...
		for(int i = 0, size = inputs.size(); i < size; i++) {
			StringEscape.escapeXml(inputs.get(i), dst);
			Assert.assertEquals(expect.get(i), dst.toString());
			dst.setLength(0);
		}
	}
	...
	public void unescapeXml() {
		...
		for(int i = 0, size = inputs.size(); i < size; i++) {
			StringEscape.unescapeXml(inputs.get(i), dst);
			Assert.assertEquals(expect.get(i), dst.toString());
			dst.setLength(0);
		}
	}
